...
const Articles = () => {
  ...
  useEffect(() => {
    const fetchArticles = async () => {
      try {
        const response = await fetch("/api/articles"); // Replace "/api/articles" with the actual API endpoint for fetching articles
        ...
        if (!contentType || !contentType.includes('application/json')) {
          setArticles([]); // Set articles to an empty array if the response is not in JSON format
          return;
        }
                ...
                >
                  ...
                  >
                    ...
                    >
                      ...
                      <p className="mt-4 text-sm sm:text-base">{article.description}</p>
